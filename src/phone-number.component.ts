...
export class PhoneNumberComponent implements OnInit, ControlValueAccessor, Validator {
  //input
  @Input()
  placeholder: string = 'Enter phone number'; //default
  @Input()
  maxlength: number = 15; //default
  ...
  private updateValue() {
    ...
    this.countryModel = this.selectedCountry;
  ...
  private updatePhoneInput(countryCode: string) {
    ...
    else this.phoneNumber = `${newInputValue.replace(/ /g, '')}`;
  }
  ...
  private static reducePrefixes(foundPrefixes: Country[]) {
    return foundPrefixes.reduce(
      (first: Country, second: Country) => (first.dialCode.length > second.dialCode.length ? first : second)
    );
  }
}
